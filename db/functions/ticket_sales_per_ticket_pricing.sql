...
SELECT e.organization_id                            AS organization_id,
       ...
       CAST(COALESCE(SUM(CASE WHEN o.status = 'Paid' THEN 1 ELSE 0 END)
                     FILTER (WHERE o.box_office_pricing IS TRUE AND o.status = 'Paid'),
                     0) AS BIGINT)                  AS box_office_order_count,
       CAST(COALESCE(SUM(CASE WHEN o.status = 'Paid' THEN 1 ELSE 0 END)
                     FILTER (WHERE o.box_office_pricing IS FALSE AND o.status = 'Paid'),
       ...
       -- Refunded count
       CAST(COALESCE(SUM(oi.refunded_quantity) FILTER (WHERE o.box_office_pricing IS TRUE AND o.status = 'Paid'),
                     0) AS BIGINT)                  AS box_office_refunded_count,
       CAST(COALESCE(SUM(oi.refunded_quantity) FILTER (WHERE o.box_office_pricing IS FALSE AND o.status = 'Paid'),
                     0) AS BIGINT)                  AS online_refunded_count,

       --Total Sold Count
       CAST(COALESCE(SUM(oi.quantity - oi.refunded_quantity)
                     FILTER (WHERE o.box_office_pricing IS TRUE AND o.status = 'Paid' AND
                                   (h.hold_type IS NULL OR h.hold_type != 'Comp')),
                     0) AS BIGINT)                  AS box_office_sale_count,
       CAST(COALESCE(SUM(oi.quantity - oi.refunded_quantity)
                     FILTER (WHERE o.box_office_pricing IS FALSE AND o.status = 'Paid' AND
                                   (h.hold_type IS NULL OR h.hold_type != 'Comp')),
