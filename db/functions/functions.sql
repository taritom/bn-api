...
SELECT e.organization_id                                                                                                                                                                     AS organization_id,
       ...
       -- Order count
       CAST(COALESCE(SUM(CASE WHEN o.status = 'Paid' THEN 1 ELSE 0 END) FILTER (WHERE p.is_box_office IS TRUE AND o.status = 'Paid'), 0) AS BIGINT)                                          AS box_office_order_count,
       CAST(COALESCE(SUM(CASE WHEN o.status = 'Paid' THEN 1 ELSE 0 END) FILTER (WHERE p.is_box_office IS FALSE AND o.status = 'Paid'), 0) AS BIGINT)                                         AS online_order_count,
       -- Actual Values
