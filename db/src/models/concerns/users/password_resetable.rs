...
pub trait PasswordResetable {
    ...
    fn find_by_password_reset_token(password_reset_token: &Uuid, conn: &PgConnection) -> Result<User, DatabaseError>;
    fn create_password_reset_token(&self, conn: &PgConnection) -> Result<User, DatabaseError>;
    fn consume_password_reset_token(token: &Uuid, password: &str, conn: &PgConnection) -> Result<User, DatabaseError>;
}
    ...
    fn has_valid_password_reset_token(&self) -> bool {
        match self.password_reset_requested_at {
            Some(password_reset_requested_at) => {
                let now = Utc::now().naive_utc();
                now.signed_duration_since(password_reset_requested_at).num_days()
