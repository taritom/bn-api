...
FROM orders o
       ...
       LEFT JOIN order_items oi_event_fees ON (oi_event_fees.item_type = 'EventFees' AND o.id = oi_event_fees.order_id)
       LEFT JOIN ticket_types tt ON (oi.ticket_type_id = tt.id)
       LEFT JOIN (SELECT order_id, ARRAY_TO_STRING(ARRAY_AGG(DISTINCT p.payment_method), ', ') AS payment_method, ARRAY_TO_STRING(ARRAY_AGG(DISTINCT p.provider), ', ') AS payment_provider FROM payments p GROUP BY p.payment_method, p.order_id) AS p on o.id = p.order_id
