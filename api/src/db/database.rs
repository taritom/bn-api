use config::Config;
use db::{ConnectionType, CacheDatabase};
...
impl Database {
    pub fn from_config(config: &Config) -> Database {
        Database {
            ...
            cache_database: CacheDatabase{inner: RedisCacheConnection::create_connection_pool(&config.redis_connection_string).ok()},
        }
    }
...
impl Clone for Database {
    fn clone(&self) -> Self {
        Database {
            ...
            cache_database: CacheDatabase{inner: self.cache_database.inner.clone()},
        }
    }
}
